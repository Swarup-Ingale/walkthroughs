...
```
lines = open("input_19").read().splitlines()
...
const lines = fs.readFileSync('input_19', 'utf8')
    .trim()
    .split('\n');

const ip = Number(lines[0].split(' ')[1]);
...
while (ptr >= 0 && ptr < prog.length) {

    r[ip] = ptr;

    const [op,a,b,c] = prog[ptr];
