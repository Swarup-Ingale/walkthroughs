...
best = min(layers, key = lambda layer: layer.count("0"))
...
for (const layer of layers) {
	if (layer.split("0").length < best.split("0").length) {
		best = layer;
	}
}

const ones = [...best].filter(x => x === "1").length;
const twos = [...best].filter(x => x === "2").length;
