...
while True:
	...
	if opcode == 99:
		...
		a = get(memory, ip + 1, mode1)
		b = get(memory, ip + 2, mode2)
		...
		outputs.append(get(memory, ip + 1, mode1))
		ip += 2
...
while (true) {
	...
	switch (opcode) {
		case 1:
			memory[memory[ip + 3]] = get(memory, ip + 1, mode1) + get(memory, ip + 2, mode2);
			ip += 4;
			break;
		...
		case 4:
			outputs.push(get(memory, ip + 1, mode1));
			ip += 2;
			break;
