...
def run (program, inputs):
	...
	while True:
		...
		if opcode == 99:
			...
			memory[memory[ip + 3]] = get(ip + 1, mode1) + get(ip + 2, mode2)
			ip += 4
			...
			memory[memory[ip + 1]] = inputs.pop(0)
			ip += 2

		elif opcode == 4:
			outputs.append(get(ip + 1, mode1))
			ip += 2

		elif opcode == 5:
			ip = get(ip + 2, mode2) if get(ip + 1, mode1) else ip + 3

		elif opcode == 6:
			ip = get(ip + 2, mode2) if not get(ip + 1, mode1) else ip + 3

		elif opcode == 7:
			memory[memory[ip + 3]] = int(get(ip + 1, mode1) < get(ip + 2, mode2))
			ip += 4
...
function run(program, inputs) {
	...
	while (true) {
		...
		switch (opcode) {
			...
			case 3:
				memory[memory[ip + 1]] = inputs.shift();
				ip += 2;
				break;

			case 4:
				outputs.push(get(ip + 1, mode1));
				ip += 2;
				break;

			case 5:
				ip = get(ip + 1, mode1) !== 0 ? get(ip + 2, mode2) : ip + 3;
				break;
			...
			case 7:
				memory[memory[ip + 3]] = get(ip + 1, mode1) < get(ip + 2, mode2) ? 1 : 0;
				ip += 4;
				break;
