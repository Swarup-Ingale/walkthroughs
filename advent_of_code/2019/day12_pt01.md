...
for moon in moons:
	potential = sum(abs(value) for value in moon["pos"])
	kinetic = sum(abs(value) for value in moon["vel"])
...
for (const moon of moons) {
	const potential = Math.abs(moon.pos[0]) + Math.abs(moon.pos[1]) + Math.abs(moon.pos[2]);
	const kinetic = Math.abs(moon.vel[0]) + Math.abs(moon.vel[1]) + Math.abs(moon.vel[2]);
